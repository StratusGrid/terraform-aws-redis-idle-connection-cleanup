...
const idleThresholdSeconds = process.env.IDLE_THRESHOLD_SECONDS
  ? process.env.IDLE_THRESHOLD_SECONDS
  : 86400; // one day
const concurrentRequestsLimit = process.env.CONCURRENT_REQUEST_LIMIT
  ? process.env.CONCURRENT_REQUEST_LIMIT
  : 50;
...
async function main(event, context) {
  ...
  for (const node of nodes) {
    ...
    try {
      ...
      redisClient = new redis(node.Port, node.Address, {
      ...
      const idleClients = clients.filter(
        (client) => client.idle > parseInt(idleThresholdSeconds)
      );
      console.log(
        `Total Clients Over ${idleThresholdSeconds} Seconds Idle Threshold: ${idleClients.length}`
      );
      ...
      try {
        if (!dry_run) {
          if (idleClient.addr) {
            ...
            console.log(
              `Successfully closed connection: ${idleClient.addr} on ${redisClient.options.host} - idle for ${idleClient.idle}`
            );
          } else {
          ...
          console.log(
            `Skipped closing connection: ${idleClient.addr} due to dry_run mode - idle for ${idleClient.idle}`
          );
        }
      } catch (error) {
